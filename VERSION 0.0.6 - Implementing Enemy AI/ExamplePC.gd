...
func _process(delta):
	...
	# If player is in the movement phase, update position to match the cursor's position
	if parent.currentAction == "movement":
		...
		newPosition.y = (0.75 * parent.map_to_world(parent.activePlayerPosition).y) + 100
		...
		match parent.activePlayerPosition:
