...
func targetActionInitializer():
	for tile in selectedAbility.abilityRange:
		...
		if ((tile.x + activeCharacter.gridIndex.x >= 0) && 
			...
			# Set the reticle location to this tile.
			# Strictly speaking, we don't HAVE to do this every time, but this is probably the simplest
			# (and fastest) option.
			reticleLoc = activeCharacter.gridIndex + tile
	# Highlight the targeted location with a special tile
